# =============================================================================
# ESCENARIO 2 — RESAMPLE A 1 HORA + MODELOS ML
...
FREQ          = 'h'
FREQ_LABEL    = '1 Hora'
LAG_LABEL     = '1 Hour'                # equivalencia de 1 lag, para gráficos en eje 'Time Lag'
HORIZON       = 168                     # pasos hacia adelante = 7 días (1 semana) a resolución 1h
HORIZON_LABEL = '7 Días (1 Semana)'
LAGS_GAP      = [1, 2, 3, 6, 12, 24, 48, 168]  # 168 h = 1 semana (lag semanal)
ROLL_WINDOWS  = [6, 24, 168]
TRAIN_RATIO   = 0.80
TRAIN_WINDOW  = 10_000                  # ventana de entrenamiento unificada (RF, XGB y SVR)
N_VIZ         = 500
...
# 6.2 Predicciones vs real
fig, ax = plt.subplots(figsize=(15, 5))
ax.plot(idx_target_test[:N_VIZ], y_test[:N_VIZ],
...
for nombre, y_pred in [('Random Forest', y_pred_rf),
    ...
    estilo = dict(linewidth=0.8, alpha=0.85)
    if nombre in ('Persistencia', 'Regresión Lineal'):
        estilo = dict(linewidth=0.9, alpha=0.7, linestyle='--')
    ax.plot(idx_target_test[:N_VIZ], y_pred[:N_VIZ],
            color=COLORES[nombre], label=nombre, **estilo)
ax.set_title(f'Escenario {ESCENARIO} — Predicciones vs Real (horizonte: {HORIZON_LABEL}, '
             f'primeros {N_VIZ} puntos del test)',
             fontweight='bold')
ax.set_ylabel('GAP (kW)')
ax.legend(framealpha=1)
...
for ax, (nombre, y_pred) in zip(axes, [('Random Forest', y_pred_rf),
    ...
    ax.scatter(y_test, y_pred, alpha=0.2, s=4, color=COLORES[nombre])
