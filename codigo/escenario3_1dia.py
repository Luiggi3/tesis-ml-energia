...
TRAIN_RATIO   = 0.80
SVR_MAX_TRAIN = None                    # sin límite: dataset pequeño (~1 400 filas)
...
X_svr, y_svr = X_train_sc, y_train_sc  # dataset pequeño, sin límite
...
ax.plot(df.index, df['GAP'].values, color='#1565C0', linewidth=1.0, alpha=0.9)
ax.axvline(idx_test[0], color='red', linestyle='--', linewidth=1.5,
           label=f'Inicio test: {idx_test[0].date()}')
ax.set_title(f'Escenario {ESCENARIO} ({FREQ_LABEL}) — GAP Diario',
             fontweight='bold')
...
ax.plot(idx_test[:N_plot], y_test[:N_plot],
        color='black', linewidth=1.5, label='Real', zorder=5)
for nombre, y_pred in [('Random Forest', y_pred_rf),
                        ('XGBoost', y_pred_xgb),
                        ('SVR', y_pred_svr)]:
    ax.plot(idx_test[:N_plot], y_pred[:N_plot],
            color=COLORES[nombre], linewidth=1.0, alpha=0.85, label=nombre)
ax.set_title(f'Escenario {ESCENARIO} ({FREQ_LABEL}) — Predicciones vs Real (conjunto de prueba completo)',
             fontweight='bold')
ax.set_ylabel('GAP (kW)')
ax.legend()
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
